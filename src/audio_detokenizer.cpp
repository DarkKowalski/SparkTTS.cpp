...
namespace spark_tts
{
    ...
    {
        const std::string bicodec_detokenizer_model_path = model_path + "/AudioDetokenizer/bicodec_detokenizer.xml";
        auto bicodec_detokenizer_model = core.read_model(bicodec_detokenizer_model_path);
        bicodec_detokenizer_ = core.compile_model(bicodec_detokenizer_model, device_name);
    }
